...
///@brief Влк\Викл реле === чет\нечет
void ReleManager::ReleOnOff(bool isOdd){
    //не парне значення(1..3..5..7)
    if (isOdd) {
        #if defined(DEBUG_MODE)
        Serial.println("🔴 (Нечетный|ODD)");
        #endif
        digitalWrite(EVEN, OFF);
        digitalWrite(ODD, ON);
    } else {
        #if defined(DEBUG_MODE)
        Serial.println("🟢 (Четный|EVEN)");
        #endif
        digitalWrite(ODD, OFF);
        digitalWrite(EVEN, ON);
    }
}

///@brief Returns true if the LED blinking process is active.
